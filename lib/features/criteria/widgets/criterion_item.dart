...
class CriterionItem extends ConsumerWidget {
  ...
  Future<void> _handleDelete(BuildContext context, WidgetRef ref) async {
    final confirmed = await ConfirmationDialog.showDeleteConfirmation(
      ...
      message: 'Are you sure you want to delete "${criterion.name}"? '
          'This will permanently remove the criterion and it will be removed from all tasks that use it. '
    ...
    if (confirmed == true && context.mounted) {
      try {
        ...
        final tasksUsingCriterion = allTasks
        ...
        if (context.mounted) {
          ...
          // Also refresh tasks list if it's being watched
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Criterion "${criterion.name}" deleted'),
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error deleting criterion: $e'),
